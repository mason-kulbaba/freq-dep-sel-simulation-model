...
#############################
#added by Yaiv 07/20/18

A <- .5

vector.of.effects <- rnorm(n = nloci, mean = 0, sd = .1)

nloci <- 10

geno<- paste(rep(c("M","D"),each = nloci) ,1: nloci)  # i would have these be 0 [no deviation from A] 1 for deviation from A. DO THIS FOR MOM EACH PARTNER OF THE MATING PAIR

geno [(1:nloci) + sample(c(0, nloci), size = nloci, replace = TRUE)]

phenotype 

A + sum(mat.geno * vector.of.effects) + sum(pat.geno * vector.of.effects)
